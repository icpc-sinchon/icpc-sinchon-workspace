...
const suapcDescription = `SUAPC는 신촌지역 6개 대학(서강, 숙명, 연세, 이화, 홍익, 한양)의
 학부생 및 대학원 1년차를 대상으로 하는 프로그래밍 대회입니다. 
 대회 문제는 서울 리저널의 문제 출제 경향을 따르며 제한시간 동안 
 얼마나 많은 문제를 정확하게 풀 수 있는지를 평가하여 순위를 결정합니다.`;

const participantDescription = `서강대학교, 숙명여자대학교, 연세대학교, 이화여자대학교, 홍익대학교, 한양대학교의 재학/휴학생인 경우
누구나 참여 가능합니다. (단, 졸업 1년 차와 대학원생은 참여 가능하되, 대회 중 스코어보드에는 보여지지 않습니다.)
`;

function getProblemIndexLabel(
  year: number,
...
function SUAPCPage() {
  ...
  const pageTitle = `SUAPC ${currentSemester.year} ${currentSemester.season}${suapcData.titleSuffix ? ` ${suapcData.titleSuffix}` : ""}`;
  ...
  const contestLinks = [
    ...
    {
      title: "문제 PDF",
      href: formatLinkURL(links.problemPdf, currentSemester),
    },
    {
      title: "해설 PDF",
      href: formatLinkURL(links.solutionPdf, currentSemester),
    },
    {
      title: "스코어보드",
      href: formatLinkURL(links.scoreboard?.[0], currentSemester),
    },
    {
      title: "공식 포스터",
      href: formatLinkURL(links.posterImage, currentSemester),
    },
    ...(showNoticePageLink
      ? [
          {
            title: "대회 유의사항",
            href: `/suapc/${currentSemester.year}-${currentSemester.season}/notice`,
          },
        ]
      : []),
  ];
          ...
          <React.Fragment key={contest.contestName}>
            ...
            <TableSection
              ...
              data={contest.problemList.map((problem, index) => ({
                ...problem,
                problemIndex: getProblemIndexLabel(
