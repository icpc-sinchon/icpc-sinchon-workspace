...
describe('SemesterService', () => {
  ...
  describe('createSemester', () => {
    test('학기를 생성하고 반환해야 합니다', async () => {
      ...
      const result = await semesterService.createSemester(createSemesterDto as any);
      expect(result).toEqual(createdSemester);
      expect(mockSemesterRepository.createSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.createSemester).toBeCalledWith({
  ...
  describe('getSemesters', () => {
    test('모든 학기를 반환해야 합니다', async () => {
      ...
      expect(mockSemesterRepository.getSemesters).toBeCalledTimes(1);
      expect(mockSemesterRepository.getSemesters).toBeCalledWith();
    });
  });

  describe('getSemesterById', () => {
    test('ID로 특정 학기를 반환해야 합니다', async () => {
      const semester = { id: 1, year: 2024, season: 'Spring', lecture: [], refundPolicy: [] };
      ...
      expect(mockSemesterRepository.getSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.getSemester).toBeCalledWith({ where: { id: 1 } });
    });

    test('존재하지 않는 학기를 조회하면 NotFoundException을 던져야 합니다', async () => {
      mockSemesterRepository.getSemester.mockResolvedValue(null);

      await expect(semesterService.getSemesterById(999)).rejects.toThrow(NotFoundException);
      expect(mockSemesterRepository.getSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.getSemester).toBeCalledWith({ where: { id: 999 } });
    });
  });
  ...
  describe('updateSemester', () => {
    test('학기를 수정하고 반환해야 합니다', async () => {
      ...
      const existingSemester = { id: 1, year: 2024, season: 'Spring', lecture: [], refundPolicy: [] };
      ...
      const result = await semesterService.updateSemester(1, updateSemesterDto as any);
  
      expect(result).toEqual(updatedSemester);
      expect(mockSemesterRepository.getSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.getSemester).toBeCalledWith({ where: { id: 1 } });
      expect(mockSemesterRepository.updateSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.updateSemester).toBeCalledWith({
        where: { id: 1 },
    ...
    test('존재하지 않는 학기를 수정하려고 하면 NotFoundException을 던져야 합니다', async () => {
      ...
      await expect(semesterService.updateSemester(999, updateSemesterDto as any))
  ...
  describe('deleteSemester', () => {
    test('학기를 삭제하고 삭제된 학기를 반환해야 합니다', async () => {
      ...
      expect(mockSemesterRepository.getSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.getSemester).toBeCalledWith({ where: { id: 1 } });
      expect(mockSemesterRepository.deleteSemester).toBeCalledTimes(1);
      expect(mockSemesterRepository.deleteSemester).toBeCalledWith({ where: { id: 1 } });
    });

    test('존재하지 않는 학기를 삭제하려고 하면 NotFoundException을 던져야 합니다', async () => {
      mockSemesterRepository.getSemester.mockResolvedValue(null);
  
      await expect(semesterService.deleteSemester(999)).rejects.toThrow(NotFoundException);
